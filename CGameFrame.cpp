...
template<class T>
float D3DXVec2Cross(const T & v1 , const T & v2 )
{
    return return v1.x * v.y2 - v1.y * v2.x;
}
...
void SphereUpdate()
{
	...
	tSegment own(D3DXVECTOR2(player.mSpriteObject.mPos.x, player.mSpriteObject.mPos.y), D3DXVECTOR2(moveVector.x, moveVector.y));
...
bool CGameFrame::Initialize(HINSTANCE aHInst, const int aCmdShow)
{
	...
	CreateHWND(aHInst, aCmdShow, "DebugWindow", D3DXVECTOR2(300, 300));
...
HWND CGameFrame::CreateHWND(HINSTANCE aHInst, const int aCmdShow)
{
	...
	if (mIsFullScreen) {
		...
		AdjustWindowRect(&rc, WS_OVERLAPPEDWINDOW | WS_VSCROLL | WS_HSCROLL | WS_VISIBLE, FALSE);
...
HWND CGameFrame::CreateHWND(HINSTANCE aHInst, const int aCmdShow, const std::string & aName, const D3DXVECTOR2 aSize)
{
	HWND result;
	result = CreateWindowEx(
		0,
